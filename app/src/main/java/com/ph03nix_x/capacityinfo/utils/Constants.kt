package com.ph03nix_x.capacityinfo.utils

object Constants {

    const val githubLink = "https://github.com/Ph03niX-X/CapacityInfo"
    const val designerLink = "https://t.me/F0x1d"
    const val romanianTranslationLink = "https://github.com/ygorigor"
    const val belorussianTranslationLink = "https://t.me/DrCyanogen"
    const val helpWithTranslationLink = "https://drive.google.com/drive/folders/1DoBUA5wye-vGw8Me6b-7Y44iXfpBSu91"
    const val telegramLink = "https://t.me/Ph03niX_X"
    const val googlePlayPackageName = "com.android.vending"
    const val exportSettingsRequestCode = 0
    const val importSettingsRequestCode = 1
}